...
BLYNK_WRITE(V0) {
    ...
    if (b_light && b_power) {
        if (state == 1 && b_power->isPowerSafe()) { 
            ...
            Blynk.virtualWrite(V8, "💡 เปิดไฟแบบ Manual แล้ว\n");
...
BLYNK_CONNECTED() {
    ...
    if (b_logger) {
        Blynk.virtualWrite(V8, "🔄 บอร์ดออนไลน์: ซิงก์ข้อมูลล่าสุดจาก Cloud สำเร็จ\n");
    }
}
...
void BlynkManager::begin(LogManager* logger, LightManager* light, PowerManager* power, TempManager* temp, FanManager* fan, TimeManager* time, String fwVer) {
    b_logger = logger; b_light = light; b_power = power; 
    b_temp = temp; b_fan = fan; b_time = time; b_fwVer = fwVer;
    
    Blynk.config(BLYNK_AUTH_TOKEN);
    if (b_logger) b_logger->sysLog("BLYNK", "Blynk IoT Initialized");
}
...
void BlynkManager::sendTelemetry() {
    if (!b_power || !b_temp || !b_fan || !b_light || !b_time) return;
    ...
    if (abs(v - last_v) >= 0.05) { 
        Blynk.virtualWrite(V2, v); 
        last_v = v;
    }

    if (abs(tBuck - last_tBuck) >= 0.5) { 
